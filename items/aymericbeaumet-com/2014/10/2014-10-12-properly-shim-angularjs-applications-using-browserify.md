...
link: https://aymericbeaumet.com/properly-shim-angularjs-applications-using-browserify/
source: aymericbeaumet-com
published: 2014-10-12T00:00:00Z
updated: 2014-10-12T00:00:00Z
first_seen: 2026-09-04T12:14:41.802665207Z
summary: 'Edit (March 5th 2015): the frontend community has evolved in the last few months and tends to be less hostile to the CommonsJS style (e.g.: Angular is now available on npm). This article has been rewritten accordingly. AngularJS is a frontend framework developed by folks at Google. It allows to build advanced web applications in a modular way by splitting the code base into small components. This brings a lot of advantages like a clean separation of concerns and an easier testability.'
content: extracted
html: 2014-10-12-properly-shim-angularjs-applications-using-browserify.html
---
...
[AngularJS](https://angularjs.org/) is a frontend framework developed by folks at Google. It allows to build advanced web applications in a modular way by splitting the code base into small components. This brings a lot of advantages like a clean separation of concerns and an easier testability.

[Browserify](http://browserify.org/) is a tool which allows to benefit from the [CommonJS](http://wiki.commonjs.org/wiki/CommonJS) modules in theoretically any JavaScript environments. Given an entry point, it computes a dependency tree, resolves it and produces a single output file. By consuming the `package.json`, Browserify enables to require `node_modules` in the build. This allows to rely on [npm](https://www.npmjs.com/) as a package manager for frontend dependencies, where [Bower](https://bower.io/) or [Component](http://component.github.io/) would have been usually used.
...
## [](https://aymericbeaumet.com/properly-shim-angularjs-applications-using-browserify/#a-sample-application)A sample application
...
```
var angular = require("angular");var app = angular.module("app", [require("angular-local-storage")]);app.run(function (localStorageService) {	if (!localStorageService.get("lastVisit")) {		angular.element("body").text("Hello stranger!");	} else {		angular.element("body").text("Welcome back!");	}	localStorageService.set("lastVisit", Date.now());});
```
...
```
npm initnpm install --save angular angular-local-storage jquerynpm install -D browserify
```
...
```
<html>	<head>		<script src="bundle.js"></script>	</head>	<body ng-app="app"></body></html>
```
...
## [](https://aymericbeaumet.com/properly-shim-angularjs-applications-using-browserify/#explanations)Explanations

There are several issues occurring here.

### [](https://aymericbeaumet.com/properly-shim-angularjs-applications-using-browserify/#modules-not-defining-an-entry-point)Modules not defining an entry point
...
### [](https://aymericbeaumet.com/properly-shim-angularjs-applications-using-browserify/#modules-not-properly-exporting-their-content)Modules not properly exporting their content
...
### [](https://aymericbeaumet.com/properly-shim-angularjs-applications-using-browserify/#modules-fetching-their-dependencies-on-the-global-context)Modules fetching their dependencies on the global context
...
One solution is to wrap the module into a function which exposes a global object with the appropriate properties. Hopefully, this could also easily done via *browserify-shim* (see above for the installation).
...
## [](https://aymericbeaumet.com/properly-shim-angularjs-applications-using-browserify/#wrap-up)Wrap up
...
A working example can be found [attached to this post](https://aymericbeaumet.com/properly-shim-angularjs-applications-using-browserify/#attachments).

## [](https://aymericbeaumet.com/properly-shim-angularjs-applications-using-browserify/#side-notes)Side notes

Some related facts which can save you time when dealing with Browserify:

- modules not published to npm can be installed by giving their git repository URL: `npm install git+https://github.com/angular-ui/ui-router.git`. Note that it is also possible to directly pass a GitHub repository: `npm install angular-ui/ui-router`. A version can be specified by appending `#tag`. See the [documentation](https://docs.npmjs.com/cli/install) for more information
- modules not containing a `package.json` cannot be installed using npm (even by directly providing a git repository). To tackle this issue, a third party installer like [napa](https://github.com/shama/napa) has to be used
...
## [](https://aymericbeaumet.com/properly-shim-angularjs-applications-using-browserify/#conclusion)Conclusion
