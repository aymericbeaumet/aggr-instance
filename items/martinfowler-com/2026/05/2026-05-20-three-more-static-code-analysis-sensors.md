...
link: https://martinfowler.com/articles/sensors-for-coding-agents.html#StaticCodeAnalysisDependencyRules
source: martinfowler-com
published: 2026-05-20T14:27:00Z
updated: 2026-05-20T14:27:00Z
...
![Mutation testing analysis diagram for mappers and related                        code](https://martinfowler.com/articles/sensors-for-coding-agents/sensors-mutation-example.png)
...
![A high level architecture diagram showing a coding agent, the sidecar, and the target codebase. The sidecar gets started via `sensors start`, loads a config file from the codebase to learn about all the sensors, and the coding agent checks the status via `sensors check`](https://martinfowler.com/articles/sensors-for-coding-agents/sensors-sidecar-setup.png)
...
![A screenshot from a terminal window showing a table listing sensors like tests, lint, typecheck, semgrep. Each row shows the interval, the overall status, a trend, when the sensor last ran, and details.](https://martinfowler.com/articles/sensors-for-coding-agents/sidecar-human-view.png)

Figure 7: Visual overview of sensor status

![A screenshot from a terminal window showing a concise textual summary for the agent of sensors like stylelint, structure, typecheck.](https://martinfowler.com/articles/sensors-for-coding-agents/sidecar-check-result.png)
...
- *Via git hooks* - For users who like their agents to do frequent small commits, a `pre-commit` hook can be a good way to force a sensors check to take place before code even gets into a commit.
- *Custom extension* - I wondered if providing the harness with a custom tool that does the check makes it more reliable. I tried this by building an extension for (and with) [Pi](https://pi.dev/). It seems quite promising, and I could easily do things like asking the user on startup if they want to start and snapshot a sensors session. But at the time of writing this, I haven't used the extension enough to tell if it's running more frequently than it would with an instruction in some markdown file.

![A screenshot from a terminal window showing the Pi agent startup screen, asking the user if they want to start the sensors Yes/No. The bottom status line of Pi shows 'sensors: not running'](https://martinfowler.com/articles/sensors-for-coding-agents/sidecar-extension-pi.png)
...
![A diagram showing the sensors sidecar and the codebase, with details of the config file showing an example of configuring lint and layers sensors. The diagram also shows how the sidecar needs parsers implemented for those types of sensors. One of the parsers is labelled 'default'](https://martinfowler.com/articles/sensors-for-coding-agents/sidecar-parsers.png)
...
![Screenshots showing vertical bar charts for multiple sensors that represent the history of a sensor's status over time. E.g., a semgrep sensor that spikes up to 9 violations in the middle of the session, then gets back to 0, then later up to 1 again, then back to 0](https://martinfowler.com/articles/sensors-for-coding-agents/sensors-history-example.png)
