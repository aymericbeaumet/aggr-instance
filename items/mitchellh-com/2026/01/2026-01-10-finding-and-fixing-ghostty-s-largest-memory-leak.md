...
content: extracted
html: 2026-01-10-finding-and-fixing-ghostty-s-largest-memory-leak.html
---

A few months ago, users started reporting that Ghostty was consuming absurd amounts of memory, with one user reporting **37 GB** after 10 days of uptime. Today, I'm happy to say [the fix has been found and merged](https://github.com/ghostty-org/ghostty/pull/10251). This post is an overview of what caused the leak, a look at some of Ghostty's internals, and some brief descriptions of how we tracked it down.[1](https://mitchellh.com/writing/ghostty-memory-leak-fix#user-content-fn-2)
...
Page 1 oldest scrollback

Page 2

Page 3

Page 4 newest active screen

The underlying "pages" are not single [virtual memory pages](https://en.wikipedia.org/wiki/Page_\(computer_memory\)) but they are a contiguous block of memory aligned to page boundaries and composed of an even multiple of system pages.[2](https://mitchellh.com/writing/ghostty-memory-leak-fix#user-content-fn-1)
...
**BUG:** metadata reset to std\_size, but mmap unchanged!
...
std\_size, assume pooled. **munmap never called!**

Standard Non-standard Leaked
...
1. This post was written without the use of AI. AI was used to assist in some of the diagrams, but they were all reviewed for correctness by a human. None of the text content was AI-generated. [↩](https://mitchellh.com/writing/ghostty-memory-leak-fix#user-content-fnref-2)

2. The reason for this is not important for this blog post, but it is an interesting detail on its own. [↩](https://mitchellh.com/writing/ghostty-memory-leak-fix#user-content-fnref-1)
