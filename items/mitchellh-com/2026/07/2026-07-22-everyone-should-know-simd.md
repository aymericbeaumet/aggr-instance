...
content: extracted
html: 2026-07-22-everyone-should-know-simd.html
---
...
I think that's wrong. SIMD can be simple to understand [1](https://mitchellh.com/writing/everyone-should-know-simd#user-content-fn-1), and common "process N values at a time" SIMD code to speed up a naive for loop almost always follows the same general shape. Once you learn the basics, writing SIMD is just about as easy as a for loop. And when it's not, it's usually a good sign to skip it for now.
...
- [Background: What Is SIMD?](https://mitchellh.com/writing/everyone-should-know-simd#background-what-is-simd)
- [The Common Shape](https://mitchellh.com/writing/everyone-should-know-simd#the-common-shape)
- [A Real Example](https://mitchellh.com/writing/everyone-should-know-simd#a-real-example)
- [Step 1: Broadcast Constants](https://mitchellh.com/writing/everyone-should-know-simd#step-1-broadcast-constants)
- [Step 2: Loop One Vector at a Time](https://mitchellh.com/writing/everyone-should-know-simd#step-2-loop-one-vector-at-a-time)
- [Step 3: Perform the SIMD Operation](https://mitchellh.com/writing/everyone-should-know-simd#step-3-perform-the-simd-operation)
- [Step 4: Reduce the Vector Result](https://mitchellh.com/writing/everyone-should-know-simd#step-4-reduce-the-vector-result)
- [Step 5: Finish with the Scalar Tail](https://mitchellh.com/writing/everyone-should-know-simd#step-5-finish-with-the-scalar-tail)
- [Recap: The Common Shape](https://mitchellh.com/writing/everyone-should-know-simd#recap-the-common-shape)
- [Why Can't the Compiler Do This?](https://mitchellh.com/writing/everyone-should-know-simd#why-cant-the-compiler-do-this)
- [Everyone Should Know SIMD](https://mitchellh.com/writing/everyone-should-know-simd#everyone-should-know-simd)
...
I have a slice of decoded codepoints that I want to consume until I see a value at or below `0xF` (a C0 control character).[2](https://mitchellh.com/writing/everyone-should-know-simd#user-content-fn-3) Terminals are *mostly* plain characters to be printed, so we try to batch all those together. So this loop finds the end of the next printable run as quickly as possible.
...
Here is the generic vector version with no CPU-specific intrinsics [3](https://mitchellh.com/writing/everyone-should-know-simd#user-content-fn-4) and no comments. I will explain it in detail later.
...
Both `values` and `threshold` are vectors, so this maps to a vector operation (a literal vector CPU instruction). The one `>` compares every lane in `values` to every corresponding lane in `threshold`. If there are eight lanes, this is equivalent to performing the scalar comparison `cps[end] > 0xF` eight times, but it does it in one CPU instruction instead.[4](https://mitchellh.com/writing/everyone-should-know-simd#user-content-fn-5)
...
Everyone should know SIMD enough to do this.[5](https://mitchellh.com/writing/everyone-should-know-simd#user-content-fn-2)

## Footnotes

1. Very impressive projects like simdutf and simdjson use extremely complex SIMD tricks to achieve their goals. But this isn't what I'd consider "everyday SIMD." [↩](https://mitchellh.com/writing/everyone-should-know-simd#user-content-fnref-1)

2. C0 controls extend beyond `0xF`. This is the cutoff Ghostty uses for this specific code path; ESC and other control-sequence handling happens elsewhere. [↩](https://mitchellh.com/writing/everyone-should-know-simd#user-content-fnref-3)

3. Generic vectors remove the CPU-specific syntax, not CPU-specific code generation. Zig still lowers these operations to the instruction set enabled for the target. Ghostty falls back to scalar code when it can't choose a supported vector width. [↩](https://mitchellh.com/writing/everyone-should-know-simd#user-content-fnref-4)

4. The comparison itself is one vector operation. Loading the vector, reducing the result, and locating the failed lane require additional instructions. The important part is that we're doing multiple comparisons at once. [↩](https://mitchellh.com/writing/everyone-should-know-simd#user-content-fnref-5)

5. This post was based on a [Lobsters comment I wrote](https://lobste.rs/s/gkcjic/simd_for_collision#c_pwdpep). [↩](https://mitchellh.com/writing/everyone-should-know-simd#user-content-fnref-2)
