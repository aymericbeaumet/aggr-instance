...
content: extracted
html: 2023-10-02-grapheme-clusters-and-terminal-emulators.html
preview:
  file: 2023-10-02-grapheme-clusters-and-terminal-emulators.preview-24f34f2cf6d7.webp
  width: 256
  height: 67
  color: '#3b3e44'
images:
- source: https://mitchellh.com/_next/image?url=https%3A%2F%2Fstatic.mitchellh.com%2Fghostty%2Fmode-2027%2Fmode2027.png&w=3840&q=75
  original:
    file: 2023-10-02-grapheme-clusters-and-terminal-emulators.image-684e7ee9feaf.png
    width: 1600
    height: 420
  variants:
  - file: 2023-10-02-grapheme-clusters-and-terminal-emulators.image-ae7f2fc22775.webp
    width: 48
    height: 13
  color: '#32363c'
---
...
Terminals operate on a grid of fixed size cells. This is fundamental to how terminal emulators operate because many [control sequences](https://en.wikipedia.org/wiki/ANSI_escape_code) programs can send to the terminal operate on *cells*.
...
Eventually, "wide characters" came along. Common wide characters are Asian characters such as 橋 or Emoji such as 😃. A function [`wcwidth`](https://www.man7.org/linux/man-pages/man3/wcwidth.3.html) was added to libc to return the width of a wide character in *cells*. Wide characters were given a width of "2" (usually). Therefore, if you type 橋 in a terminal emulator, the character will take up two grid cells and your cursor should jump forward by two cells.
...
What's with the zero-width character? The codepoint `U+200D` is known as a [Zero-Width Joiner](https://en.wikipedia.org/wiki/Zero-width_joiner) (ZWJ) and has a standards-defined width of zero. The ZWJ tells text processing systems to treat the codepoints around it as *joined* into a single character. That's why you can type both "🧑‍🌾" and "🧑🌾"; the only difference between these two quoted values is the farmer on the left has a zero-width joiner between the two emoji.
...
Aside: font shaping. Grapheme clustering only solves the problem of determining grapheme boundaries in a stream of codepoints. It does not solve the problem of *rendering* a stream of codepoints. For that, you need a *font shaper* such as [Harfbuzz](https://harfbuzz.github.io/). Harfbuzz sees a stream of codepoints, detects the graphemes, and is able to then map those graphemes to individual glyphs in a font.
...
![](https://mitchellh.com/_next/image?url=https%3A%2F%2Fstatic.mitchellh.com%2Fghostty%2Fmode-2027%2Fmode2027.png&w=3840&q=75)
