...
updated: 2026-09-25T21:09:27Z
first_seen: 2026-09-26T00:33:08.793159958Z
authors:
- specked-citrus
summary: 'Article URL: https://swarmtraces.org/ Comments URL: https://news.ycombinator.com/item?id=49849985 Points: 130 # Comments: 85'
content: extracted
html: 2026-09-25-revealing-the-details-of-how-openai-agents-hacked-hugging.html
preview:
  file: 2026-09-25-revealing-the-details-of-how-openai-agents-hacked-hugging.preview-dcf399df159d.webp
  width: 256
  height: 134
  alt: Revealing the details of how OpenAI agents hacked Hugging Face
  color: '#efefee'
images:
- source: https://swarmtraces.org/assets/opengraph-preview.jpg?v=77a1f50796
  original:
    file: 2026-09-25-revealing-the-details-of-how-openai-agents-hacked-hugging.image-e3fcaea85387.jpg
    width: 1200
    height: 630
  color: '#fcfcfa'
- source: https://swarmtraces.org/article/figures/chain-assembly.png
  original:
    file: 2026-09-25-revealing-the-details-of-how-openai-agents-hacked-hugging.image-21c27fcabe34.png
    width: 1600
    height: 874
  color: '#fafbfb'
---
...
[Explore the data](https://swarmtraces.org/viewer/)
...
![The screenshot service’s browser follows the shortened links, collects the code chunks, then decodes and runs the full program.](https://swarmtraces.org/article/figures/chain-assembly.png)\
...
Many recovered payloads contain code to delete files initially used to deliver or trigger programs on Hugging Face workers. In one example targeting REDACTED\_ORG\_NAME/hf-exports, the delivered program contains the comment:
...
A separate payload matches FALL to a credential labeled: EXPORT\_REDACTED\_ORG\_NAME\_HF\_TOKEN.
...
The supplied SHA and size match the repository’s 8-bit TensorFlow Lite GPT-2 weights [2](https://swarmtraces.org/#fn-2).
...
The script chooses which token candidates to use for further reconnaissance. A function called \`rule\_score\` assigns points to each token based on the listed permissions.
...
[slackcyber.json](https://swarmtraces.org/viewer/#/row/R0189572):

‘cybergym’

[slackbatch.gz](https://swarmtraces.org/viewer/#/row/R0189573):

‘exploitgym’, 'restart\_server’, ‘catflag’, ‘arvo\_20494’, ‘ff79aaeb2d57’, ‘cybergym-server’, ‘cybergym-exploit’, ‘target\_server’, ‘packages.hub’, ‘controller\_stub’

[slackbatch2.gz](https://swarmtraces.org/viewer/#/row/R0189574):
...
[slackd3.gz](https://swarmtraces.org/viewer/#/row/R0189575):

‘d293315c730d’, ‘arvo\_61677’, ‘61677’, ‘rollout died’
...
The resulting compact list is stored to OUT\[‘term\_hits’\].\
...
5. **We can’t confirm whether all of our dataset is activity from OpenAI agents.** As this data came from millions of link shortener links, we cannot be sure that the data within our dataset is entirely from this swarm. If a URL matched an encoding schema we discovered, it may have inadvertently ended up in our dataset. When possible, we have cross referenced agent names, file contents and estimated dates with the data OpenAI has released to ensure the evidence presented likely originated from this swarm. We’ve flagged parts of our analysis that differ from the timeline published by OpenAI as well as the timeline published by METR and Redwood.
   ...
   We also observed traffic from agents that did not seem related to this event, but we think were part of the same swarm as detailed in collusion.wiki and self-identified as agents from OpenAI.[4](https://swarmtraces.org/#fn-4) Even if we were to assume all of our data is traffic from agents, we cannot say for certain what traffic originates from the swarm that attacked Hugging Face as this dataset may include traffic from any agents or humans who use link shorteners, and other agent swarms using the same link shortening trick. We’ve made efforts to separate these payloads out where possible.
