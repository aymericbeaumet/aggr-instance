...
link: https://www.tigrisdata.com/blog/objgit-packfiles/
source: hnrss-org-frontpage
published: 2026-09-16T17:16:11Z
updated: 2026-09-16T17:16:11Z
first_seen: 2026-09-19T16:46:35.337179398Z
authors:
- evacchi
summary: 'Article URL: https://www.tigrisdata.com/blog/objgit-packfiles/ Comments URL: https://news.ycombinator.com/item?id=49730069 Points: 101 # Comments: 23'
content: extracted
html: 2026-09-16-you-can-run-git-on-object-storage-if-you-re-make-packfiles.html
preview:
  file: 2026-09-16-you-can-run-git-on-object-storage-if-you-re-make-packfiles.preview-dc78b05e3ee5.webp
  width: 256
  height: 144
  color: '#1a2235'
images:
- source: https://www.tigrisdata.com/blog/assets/images/hero-image-81fc688a5cf1e95701d66a80813da3c1.webp
  original:
    file: 2026-09-16-you-can-run-git-on-object-storage-if-you-re-make-packfiles.image-01b7912a2cd6.webp
    width: 2400
    height: 1350
  variants:
  - file: 2026-09-16-you-can-run-git-on-object-storage-if-you-re-make-packfiles.image-242f5ef6eb41.webp
    width: 320
    height: 180
  - file: 2026-09-16-you-can-run-git-on-object-storage-if-you-re-make-packfiles.image-e71316be60d8.webp
    width: 640
    height: 360
  color: '#0f172b'
- source: https://www.tigrisdata.com/blog/assets/images/delta-brain-23a5172b6245e09bda8c9b7b65fd4e48.webp
  original:
    file: 2026-09-16-you-can-run-git-on-object-storage-if-you-re-make-packfiles.image-04e58095593d.webp
    width: 385
    height: 375
  color: '#fafafa'
---
...
Either way, I’m building a Git server backed by object storage as an [open-source project](https://github.com/tigrisdata/objgit). It sounded simple enough to start: Git looks like a filesystem, so let’s use a filesystem as a translation layer on top of object storage to make Git speak object storage. This model worked… [ok, I guess?](https://www.tigrisdata.com/blog/objgit/) But it didn’t work for real-world size repositories, so I needed a different approach. Git stores everything in [Objects](https://git-scm.com/book/en/v2/Git-Internals-Git-Objects), so why not store those as objects in Tigris?

Turns out Git packfiles and how they intersected with my (admittedly somewhat terrible) filesystem shim were the main reason why it was slow. I ended up having to invent my own packfile format with a columnar store that’s object storage native. This is the fruit of all of my performance analysis, metrics annotations, and more [Texas-style distributed systems work](https://www.usenix.org/system/files/1311_05-08_mickens.pdf) than you can make your k8s cluster shake sticks at.
...
[​](https://www.tigrisdata.com/blog/objgit-packfiles/#what-is-a-git-a-miserable-little-pile-of-objects "Direct link to what-is-a-git-a-miserable-little-pile-of-objects")
---
...
Imagine Git as two things: a sea of objects and named references to individual objects. Each object is a [content-addressed](https://en.wikipedia.org/wiki/Content-addressable_storage) and compressed file. Here’s an example from a tiny git repository:
...
```
$ cd ~/Code/linux.git/$ tree objectsobjects├── info└── pack    ├── pack-45986f41063f286029742ec12e2c2882b88c5786.idx    ├── pack-45986f41063f286029742ec12e2c2882b88c5786.pack    └── pack-45986f41063f286029742ec12e2c2882b88c5786.rev3 directories, 3 files
```

Yeah, as I’m sure you guessed just putting everything into their own files won’t scale to something like the Linux kernel. I’m pretty sure you’d run into inode limits like everyone did in the era of [fractal `node_modules` folders](https://github.com/npm/npm/issues/11747).
...
Git works around this by putting objects into [packfiles](https://git-scm.com/book/en/v2/Git-Internals-Packfiles), compressed bundles of objects that store them all in the same file. Here's an example of the packfile efficiency in my checkout of objgit:

```
$ git count-objects -vcount: 756size: 3500in-pack: 448packs: 1size-pack: 321prune-packable: 0garbage: 0size-garbage: 0
```

If you ever need to “force” git to put bare objects into a packfile, you can run `git gc`:

```
$ git gc[omitted for brevity]$ git count-objects -vcount: 0size: 0in-pack: 1203packs: 2size-pack: 848prune-packable: 0garbage: 0size-garbage: 0
```
...
## Yo dawg, herd you like objects [​](https://www.tigrisdata.com/blog/objgit-packfiles/#yo-dawg-herd-you-like-objects "Direct link to Yo dawg, herd you like objects")

Here's the object count for a copy of the Linux kernel:

```
xe@zohar:~/Code/linux.git$ git count-objects -vcount: 0size: 0in-pack: 11827138packs: 1size-pack: 3876775prune-packable: 0garbage: 0size-garbage: 0
```
...
### Messin' with Packfiles [​](https://www.tigrisdata.com/blog/objgit-packfiles/#messin-with-packfiles "Direct link to Messin' with Packfiles")
...
```
$ git gc # force objects into a packfile$ git verify-pack -v .git/objects/pack/pack-3971f5085c23c38be00e517ed0c64ca7df19b746.idx1c7a26a901724b4ce766655ac387413fb9ec7966 commit 526 366 129cc9867337c2ebae85ba2350f901e0bcc209fe26 blob   13 22 3788e67afbb2ee6bdcbb79061dfdfb93febce857bd3 tree   37 48 400non delta: 3 objects.git/objects/pack/pack-3971f5085c23c38be00e517ed0c64ca7df19b746.pack: ok
```
...
```
  $ git count-objects -v          .git/objects/pack/  count:            0             └── pack-45986f41..c5786.pack   3.7 GiB  in-pack:   11827138  packs:            1             eleven million loose files would be  size-pack:  3876775             eleven million inodes. so: one file.   .idx                             .pack  ┌───────────────────────┐       ┌──────────────────────────────────────┐  │ 0002ff4c..  0x0000c   │       │███ ██ ████ █ ███████ ██ █ ████ ██████│  │ 0031ab90..  0x0a13f   │       └──────────────────────────────────────┘  │ 1c7a26a9..  0x1f3a4   │  │ ...                   │  └───────────────────────┘  each row's colour is the run of bytes its offset points at, so a  read is a seek to an offset inside one very big file
```

ALL/03 three rows, three offsets, three runs of bytes

The great part is that this works really well when everything is in a filesystem. Git [mmaps](https://en.wikipedia.org/wiki/Mmap) the packfiles so that the kernel treats disk contents as memory pages, meaning that trying to read past what’s “in memory” makes the kernel load it instead of userspace. This is faster than loading it from the disk directly. It’s a shame this design doesn’t work in object storage.

### If only you could construct Range requests from packfiles [​](https://www.tigrisdata.com/blog/objgit-packfiles/#if-only-you-could-construct-range-requests-from-packfiles "Direct link to If only you could construct Range requests from packfiles")

At some level this sounds pretty great for object storage, right? You have offsets into the packfiles and then you can “just” grab out a single object from a packfile with an [HTTP Range request](https://developer.mozilla.org/en-US/docs/Web/HTTP/Guides/Range_requests) right? Objects are placed randomly within packfiles and other attempts at storing Git in object storage end up having problems here. Tigris is really good at random access scans, so most of the hard part is figuring out how to grab the right data out of the bucket.
...
FIG 03 One ranged GET, 366 bytes out of the middle of 128 MiB
...
## SEND CUE SHEET [​](https://www.tigrisdata.com/blog/objgit-packfiles/#send-cue-sheet "Direct link to SEND CUE SHEET")
...
However there’s one catch that kinda ruins this easy way to back up CDs: they can store *multiple* recording sessions on the same disc. Most of the time this wasn’t used outside of making piracy on certain late 90’s/early 00’s game consoles more annoying, but there was [that one Ricoh Encryptease product](https://youtu.be/_5ucImqdKbY) that combined a user-recordable area with a factory printed area so that you could encrypt files on CDs you share with the decryption software shipping alongside it. This is about as cursed as it sounds.
...
Wait, I know what you’re thinking. You’re thinking that I’m about to make a [Chesterton’s Fence violation](https://en.wikipedia.org/wiki/Wikipedia:Chesterton%27s_fence). Just “rolling my own” format for something as dear and precious as storing the revision history of a company’s code repositories is probably one of the worst decisions you can make, right?
...
![Expanding brain meme. Small brain: git stores diffs against an empty folder. Bigger brain: git stores the entire files for every version. Galaxy brain: git stores diffs against an empty folder.](https://www.tigrisdata.com/blog/assets/images/delta-brain-23a5172b6245e09bda8c9b7b65fd4e48.webp)

### Objgit's packfile format that probably needs a name [​](https://www.tigrisdata.com/blog/objgit-packfiles/#objgits-packfile-format-that-probably-needs-a-name "Direct link to Objgit's packfile format that probably needs a name")
...
```
  git wants four objects out of packs/019a7f3c..bin                       A        B             C          D                       ▼        ▼             ▼          ▼              ┌────────────────────────────────────────────────┐  container   │░░░░░░█████░░░░█████░░░░░░░░░█████░░░░░░█████░░░│              └────────────────────────────────────────────────┘  five requests go out at once:  A   16 MiB  │      ░░░░░                                     │  in flight  B   40 MiB  │               ░░░░░                            │  in flight  C   77 MiB  │                             ░░░░░              │  in flight  D  107 MiB  │                                        ░░░░░   │  in flight  whole file  │░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░│  in flight  four ranged GETs race the background download of the whole  file. whatever the download reaches stops needing a GET.
```

01/07 128 MiB of packfile; git wants A, B, C and D out of it

note

There’s nothing in the definition of the packfile format that limits packfiles to 128Mi, I’m just doing that to prevent them from getting too big to download quickly. In theory if you store a large binary blob (such as 3d models, perfectly legal backups, etc) into the git repository directly it could result in a packfile that’s bigger than 128Mi. I plan to solve this by implementing [Git Large File Storage](https://git-lfs.com/) in the near future.
...
## The funny numbers [​](https://www.tigrisdata.com/blog/objgit-packfiles/#the-funny-numbers "Direct link to The funny numbers")
...
- objgit itself: [https://github.com/tigrisdata/objgit](https://github.com/tigrisdata/objgit), a newer repository with some AI assisted development for prototyping.
- My e/x/perimental monorepo: [https://github.com/Xe/x/](https://github.com/Xe/x/), a repo that's been around for over a decade and holds many experiments.
- The Tigris blog repo: [https://github.com/tigrisdata/tigris-blog](https://github.com/tigrisdata/tigris-blog), a repo that has a lot of images and other incompressible objects in it.
...
### Push tests [​](https://www.tigrisdata.com/blog/objgit-packfiles/#push-tests "Direct link to Push tests")
...
FIG 07 S3 requests to push, before and after the format change (log scale)
...
FIG 08 Wall time to push, log scale, speedup on the right
...
### Clone tests [​](https://www.tigrisdata.com/blog/objgit-packfiles/#clone-tests "Direct link to Clone tests")

I also wanted to see how the difference affected clone times. There were the same benefits as with pushing:

FIG 09 S3 requests to clone, before and after the format change (log scale)
...
FIG 10 Wall time to clone, log scale, speedup on the right
...
## Conclusion section [​](https://www.tigrisdata.com/blog/objgit-packfiles/#conclusion-section "Direct link to Conclusion section")

I’m still actively working on this. I’m not confident enough to use this for my own projects yet and I wouldn’t blame you for not wanting to use this yet either. I still haven’t implemented authentication, authorization, any kind of API (my long-form [SigV4 auth post](https://www.tigrisdata.com/blog/sigv4/) was actually going to be an objgit post!), or any rate limit beyond what your machine can physically process. If you were to take this, run it, and then expose it to the Internet, then anyone that can connect to that server can pull or push whatever they want. Consider not doing that.
